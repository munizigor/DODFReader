...
resource "aws_s3_object" "my_deployment_package_zip" {
  ...
  key    = "my_deployment_package2.zip"   # Substitua pelo nome desejado do arquivo .zip no bucket
  source = "./my_deployment_package2.zip" # Substitua pelo caminho local do arquivo .zip

  etag = filemd5("./my_deployment_package2.zip")
}
